...
class Voronoi:
    ...
    def search_path(self,maze=None, s=None, g=None, neighborhood=8):
        """
        ...
        sbからsgまでのボロノイ境界上を移動する経路をbfsで探索する。その際、生成源上を通らない経路を求める。
        ...
        """
        time_start = time.perf_counter()
        def find_voronoi(point,diagram=None):
            if diagram  is None:
                diagram = self.diagram
            x,y = point
            region_index = diagram[y,x] 
            voronoi_points = []
            for v_point in self.V:
                x_2,y_2 = v_point
                if diagram[y_2,x_2] == region_index:
                    voronoi_points.append(v_point)
        ...
        i = 0
        sw = 0
        for column in self.check_overlap.T: #生成源の値を3とする
            j = 0
            for value in column:
                if value != 0:
                    maze[j,i] = 3
        ...
        if g is None:
            g=(self.W - 1, self.H - 1)
        s = [coord - 1 if coord == self.W or coord == self.H else coord for coord in s] #サイズギリギリに設定した時に -1　にして更新する。
        ...
        def bfs(maze, start,goal, neighborhood=8):
            """
            s,gの最寄りの道を探す。
            ...
            dx,dy = 1,0
            que_path = deque()
            voronoi_pointss = find_voronoi((start[0]  ,start[1] ))
            
            if len(voronoi_pointss) == 0: #ボロノイ点がなし
                print('ボロノイ点がありません')
                return None,None,None
            sorted_points = sorted(voronoi_pointss, key=lambda p: math.sqrt((p[0] - goal[0]) ** 2 + (p[1] - goal[1]) **2))
        ...
        # bfsでs,gから最寄りの最短経路を求める
        sb, path_sb,main_s = bfs(maze, s,g, neighborhood)
        if sb == None or path_sb == None:
            return None
        #print(path_sb)
        #path_sb = path_sb[1]
        for x,y in path_sb:
        ...
        sw = 1
        print('start終了')
        gb, path_gb,main_g= bfs(maze, g, s,neighborhood)
        if gb == None or path_gb == None:
            return None
        ...
        while que:
            ...
            for i in range(neighborhood):
                ...
                if nx < 0 or self.W <= nx or ny < 0 or self.H <= ny:
                    continue;
                if maze[ny, nx] == 0: # 0 is not boundary
                    continue;
                if parent[ny][nx] != -1:
                    continue;
                if self.check_overlap[ny, nx] != 0:  # 経路が生成源の上を通る時に、生成源上を避ける (Yabe)
                    continue;
                que.append((nx, ny))
                #main += calculate_distance(que[0], que[1])
                parent[ny][nx] = now
        ...
        for i in range(1, len(path)):
            ...
            cv2.line(ex_img, start, end, (0, 0, 255, 255), thickness=3)
    ...
    def edge_straight(self):
        """
        ボロノイ境界を直線に置き換える。 (Yabe)
        画面端とボロノイ境界の交点とボロノイ頂点をcv2.lineで繋ぐ。
        理想頂点self.Vmの周囲4座標のうち、共通するボロノイ領域が2つある場合、cv2.lineで繋ぐ。
        self.Eを書き換える。self.Emの書き換えは実装していない。

        Returns
        -------
        newEset : set of tuple of int
            直線の集合
        """

        # ボロノイ頂点をVm_listに抽出する
        Vm_list = list(self.Vm)

        # ボロノイ境界と画面端の交点をVm_listに抽出する
        for v in self.Em:
            for i in range(2):
                if v[i][0] == 0.5:
                    Vm_list.append((v[i][0] + 1, v[i][1]))
                elif v[i][0] == self.W - 0.5:
                    Vm_list.append((v[i][0] - 1, v[i][1]))
                elif v[i][1] == 0.5:
                    Vm_list.append((v[i][0], v[i][1] + 1))
                elif v[i][1] == self.H - 0.5:
                    Vm_list.append((v[i][0], v[i][1] - 1))
        ...
        dx = [0.5, -0.5, 0.5, -0.5]
        dy = [0.5, 0.5, -0.5, -0.5]
        newE = np.zeros((self.H, self.W), np.uint8)
        Vm_len = len(Vm_list)

        # Vm_listの点をcv2.lineで繋ぐ
        for i in range(Vm_len):
            for j in range(Vm_len):
                if i >= j:
                    continue;
                ...
                vm2 = Vm_list[j]
                for m in range(4):
                    a = (int(vm1[0] + dx[m]), int(vm1[1] + dy[m]))
                    v1.append(a)
                    v1color.add(self.diagram[a[1], a[0]])
                    b = (int(vm2[0] + dx[m]), int(vm2[1] + dy[m]))
                    v2.append(b)
                    v2color.add(self.diagram[b[1], b[0]])

                # その2点を直線で繋ぐ
                v_intersection = v1color & v2color
                if len(v_intersection) == 2: # ボロノイ頂点は、理想頂点を囲む座標4つで1まとまり -> 同じ色を2つ持つ頂点を繋ぐ
                    for m in range(4):
                        for n in range(4):
                            cv2.line(newE, v1[m], v2[n], 1, thickness=1)
